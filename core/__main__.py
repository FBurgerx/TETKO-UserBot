...
def _parse_args():
    ...
    p = argparse.ArgumentParser(description="TETKO Kernel")
    p.add_argument(
        "--no-web",
        dest="no_web",
        action="store_true",
        default=os.environ.get("TETKO_NO_WEB", "0") == "1",
        help="Disable the web panel (env: TETKO_NO_WEB=1). Panel is ON by default.",
    )
    p.add_argument(
        "--proxy-web",
        dest="proxy_web",
        default=os.environ.get("TETKO_PROXY_WEB", ""),
        help="Enable web proxy at specified path (e.g., /web or /). Use env: TETKO_PROXY_WEB=/web",
    )
    p.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("TETKO_PORT", 8080)),
        help="Web panel port (default: 8080, env: TETKO_PORT)",
    )
    p.add_argument(
        "--host",
        default=os.environ.get("TETKO_HOST", "127.0.0.1"),
        help="Web panel host (default: 127.0.0.1, env: TETKO_HOST)",
    )
    p.add_argument(
        "--core",
        dest="core",
...
async def _main() -> None:
    ...
    # Start with a clean terminal; the lifecycle UI draws the fixed banner.
