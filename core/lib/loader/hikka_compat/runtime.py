...
class _HikkaClientProxy:
    """Small Hikka-only facade that adds client.tg_id to TETKO ClientProxy."""
...
class InlineProxy:
    ...
    def _strip_callbacks_for_mcub(self, markup) -> list:
        """Return a copy of prepared markup safe to pass to TETKO inline_form.

        Two transforms are applied to each callback button:

        1. Remove the callable ``callback`` key - TETKO's inline_form re-generates
        ...
        2. Rename ``callback_data`` → ``data`` - TETKO's button-building helpers
           (e.g. InlineManager._format_telethon_buttons) read spec.get("data"),
           not ``callback_data``.  Without this rename the token is silently
           dropped and Telethon-TETKO falls back to using the button text as
           callback data.
        """
        if not markup:
            return markup
