...
def _ensure_fake_package() -> str:
    ...
    try:
        from core.lib.loader.module_config import (
            ConfigValue as _TETKOConfigValue,
        )
        from core.lib.loader.module_config import (
            Validator as _TETKOValidator,
        )
        ...
        class _HikkaCompatibleConfigValue:
            def __init__(
                ...
                self.validator = validator or _TETKOValidator()
                self.on_change = on_change
                self._value = None
                self.hidden = getattr(validator, "hidden", False)
                self._config_value = _TETKOConfigValue(
                    key=option,
                    default=default,
        ...
        class _HikkaCompatibleModuleConfig(dict):
            ...
            def from_dict(self, data: dict) -> None:
                """Hydrate config using the TETKO ModuleConfig method name."""
