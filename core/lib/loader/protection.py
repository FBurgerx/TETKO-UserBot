...
class ModuleProtectionPolicy:
    """Centralized trust policy for TETKO module loading.
    ...
    def assert_system_manifest_allowed(
        ...
        # Tek is a first-class TETKO system module.  It is intentionally allowed
        # to evolve without being blocked by a stale manifest hash.  Its path and
        # manifest declaration are still checked above, so arbitrary files cannot
        # use this exception.
        if clean_name.casefold() == "tek":
            return
