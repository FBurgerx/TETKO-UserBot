...
class ConfigManager:
    ...
    def _get_backup_path(self, cfg: dict) -> str:
        """Get backup file path ~/.TETKO/{hash}/.backup-config.json."""
        api_hash = self._get_api_hash(cfg)
        mcub_dir = os.path.expanduser(f"~/.TETKO/{api_hash}")
        return os.path.join(mcub_dir, self.BACKUP_FILENAME)

    def _save_backup(self, cfg: dict) -> bool:
        """Save config to ~/.TETKO/{hash}/.backup-config.json."""
        try:
            api_hash = self._get_api_hash(cfg)
            if not api_hash:
                return False
    ...
    def _load_backup(self, cfg: dict) -> dict | None:
        """Load config from ~/.TETKO/{hash}/.backup-config.json."""
        # If we have api_id/api_hash, look in specific folder
        api_hash = self._get_api_hash(cfg)
        if api_hash:
            backup_path = os.path.expanduser(
                f"~/.TETKO/{api_hash}/{self.BACKUP_FILENAME}"
            )
            if os.path.exists(backup_path):
                return self._load_backup_file(backup_path)
            return None

        # No api_hash - search all folders in ~/.TETKO/
        mcub_home = os.path.expanduser("~/.TETKO")
    ...
    def _env_config() -> dict[str, Any]:
        ...
        if os.environ.get("TETKO_API_ID"):
            env_config["api_id"] = os.environ["TETKO_API_ID"]
        if os.environ.get("TETKO_API_HASH"):
            env_config["api_hash"] = os.environ["TETKO_API_HASH"]
        if os.environ.get("TETKO_PHONE"):
            env_config["phone"] = os.environ["TETKO_PHONE"]
        return env_config
        ...
        while True:
            try:
                ...
                print(
                    ...
                    "   Run TETKO with TETKO_NO_WEB=0 and open the web setup wizard\n"
