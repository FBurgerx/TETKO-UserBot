...
_LOG_FILE = os.path.join(_LOG_DIR, "kernel.log")
...
class _WarningConsoleFilter(logging.Filter):
    """Allow records at the configured console level and above."""
...
class _ColorConsoleFormatter(logging.Formatter):
    """Compact ANSI console formatter. File logs remain detailed."""
    ...
    def format(self, record: logging.LogRecord) -> str:
        color = self._COLORS.get(record.levelno, "\033[97m")
        ...
        return f"{color}[{mark}] {message}{self._RESET}"
...
_TETKO_ROOTS: dict[str, str] = (
    {}
)  # abs_path  →  logical prefix (system / custom_modules)


def _build_mcub_roots() -> None:
    """Lazily populate _TETKO_ROOTS from the kernel instance if available."""
    if _TETKO_ROOTS:
        return
    for name, mod in list(sys.modules.items()):
        ...
        if loaded:
            _TETKO_ROOTS[os.path.abspath(loaded)] = "custom_modules"
        if system:
            _TETKO_ROOTS[os.path.abspath(system)] = "system"
        break
...
def path_to_module(filepath: str) -> str:
    ...
    _build_mcub_roots()
    for root_abs, prefix in _TETKO_ROOTS.items():
        sep = root_abs + os.sep
