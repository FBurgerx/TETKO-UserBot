...
def _normalize_inline_message_id(value: Any) -> Any:
    ...
    ``InputBotInlineMessageID``/``InputBotInlineMessageID64`` object. Older TETKO
...
def _inline_buttons(buttons: Any) -> Any:
    """Normalize TETKO inline button rows for Telethon edit requests."""
    if buttons is None:
        return None
...
class InlineMessage:
    """Native TETKO inline message with edit/delete/answer API.
