...
def _get_mcub_dir_path(api_id: int, api_hash: str) -> str:
    """Return TETKO data directory path without creating it."""

    key = f"{api_id}{api_hash}"
    instance_hash = hashlib.sha256(key.encode()).hexdigest()[:16]
    return os.path.expanduser(f"~/.TETKO/{instance_hash}")


def _ensure_not_symlink(path: str) -> None:
    """Refuse sensitive TETKO paths that are symlinks."""
...
def get_mcub_dir(api_id: int, api_hash: str) -> str:
    """Get/create TETKO data directory based on API credentials.

    Returns:
        Path to $HOME/.TETKO/{hash(API_ID+API_HASH)[:16]}
    """
...
def session_exists(api_id: int, api_hash: str) -> bool:
    ...
    so it must not create empty ``~/.TETKO/<hash>`` directories for credentials
...
def lock_sensitive_files(
    ...
    Args:
        mcub_dir: TETKO directory for resolving relative paths.
...
def audit_permissions(mcub_dir: str, logger=None) -> dict:
    """Audit all files in TETKO directory for correct permissions.

    Args:
        mcub_dir: TETKO directory to audit.
