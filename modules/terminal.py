...
MAX_OUTPUT = 3500
CMD_TIMEOUT = 30
BLACKLIST = ['rm\\s+(-[a-zA-Z]*\\s+)*-[a-zA-Z]*[rR][a-zA-Z]*f?\\s+/\\s*$', 'rm\\s+(-[a-zA-Z]*\\s+)*-[a-zA-Z]*[rR][a-zA-Z]*f?\\s+/\\*', 'rm\\s+(-[a-zA-Z]*\\s+)*-r[f]?\\s+~', 'rm\\s+(-[a-zA-Z]*\\s+)*-r[f]?\\s+/sdcard', '\\bmkfs\\b', '\\bdd\\s+.*of=/dev/', '\\bdd\\s+.*if=/dev/(zero|random|urandom)', ':\\(\\)\\s*\\{\\s*:\\|:&\\s*\\};:', '>\\s*/dev/(sd|block|mmcblk)', '\\b(shutdown|reboot|halt|poweroff)\\b', '\\binit\\s+0\\b', '\\bkill\\s+-9\\s+1\\b', '\\bkillall\\s+python\\b', '\\bpkill\\s+.*python\\b', '^\\s*su(\\s|$)', '^\\s*sudo(\\s|$)']
...
def _clean_error(text: str) -> str:
    ...
    for line in text.split('\n'):
        line = _re.sub('^/[^:]*sh:\\s*\\d+:\\s*', '', line)
        line = _re.sub('^/data/[^:]+:\\s*\\d+:\\s*', '', line)
        if line.strip():
...
class TerminalModule(Module):
    ...
    @command(name='t', aliases=['term', 'sh'], description='Выполнить shell-команду', only_for='owner')
    async def cmd_terminal(self, event, args):
        """Выполнить shell-команду, отправить вывод в чат."""
        if not args:
            await self.respond(event, '🖥 <b>Terminal</b>\nИспользование: <code>.t &lt;команда&gt;</code>\nПример: <code>.t ls -la</code>', parse_mode='html')
            return
        cmd = ' '.join(args)
        reason = _is_blocked(cmd)
        if reason:
            await self.respond(event, f'🚫 <b>Команда заблокирована</b>\n<code>{_esc(cmd)}</code>\n\n<i>Совпал запрещённый паттерн:</i>\n<code>{_esc(reason)}</code>', parse_mode='html')
            log.warning(f'Terminal: blocked command: {cmd!r} (pattern: {reason})')
            return
        header = f'<blockquote><b>$</b> <code>{_esc(cmd)}</code></blockquote>\n\n\n'
        await self.respond(event, header + '<i>executing…</i>', parse_mode='html')
        try:
            ...
            try:
                ...
                proc.kill()
                await proc.wait()
                await self.respond(event, header + '<b>❌ Error:</b>\n' + f'<blockquote><code>timeout ({CMD_TIMEOUT}s)</code></blockquote>', parse_mode='html')
                return
            ...
            await self.respond(event, header + '<b>❌ Error:</b>\n' + f'<blockquote><code>{_esc(err_clean)}</code></blockquote>', parse_mode='html')
        except Exception as e:
            log.exception('terminal command failed')
            await self.respond(event, header + '<b>❌ Error:</b>\n' + f'<blockquote><code>{_esc(str(e))}</code></blockquote>', parse_mode='html')