...
# Что включаем в бэкап (относительно корня юзербота)
INCLUDE_PATHS = [
    "config.json",
    "data/tetko_db",
    "data/tetko_config",
...
# Сессия — полный доступ к аккаунту. Только по явному разрешению.
...
class Backup(Module):
    ...
    config = {
        "auto_interval_hours": 0,   # 0 = авто-бэкапы выключены
        "include_session": False,   # НЕ включать сессию по умолчанию
        "notify": True,
    }
    ...
    def _make_backup(self, include_session: bool = False) -> Path | None:
        ...
        # удаляем старые, оставляем MAX_LOCAL_BACKUPS свежих
        self._rotate_local()
        return out_path
    ...
    def _rotate_local(self) -> None:
        ...
        try:
            backups = sorted(
                BACKUP_DIR.glob("tetko_backup_*.tar.gz"),
    ...
    async def cmd_backup(self, event, args):
        ...
        # .backup list — показать локальные бэкапы
        if action == "list":
            backups = self._list_local()
            if not backups:
                await event.edit("📂 <b>Бэкапы</b>\n\nПусто.", parse_mode="html")
                return
            lines = []
            for b in backups:
        ...
        if out_path is None:
            await event.edit("❌ Не удалось создать бэкап.")
            return
        ...
        if action == "local":
            await event.edit(
                f"✅ <b>Бэкап создан</b>\n\n"
                f"▫️ <code>{out_path}</code>\n"
                f"▫️ Размер: <code>{size_kb} KB</code>{note}",
                parse_mode="html",
            )
            return
        ...
        try:
            await self.client.send_file(
                ...
                caption=(
                    f"🗃 <b>Бэкап TETKO</b>\n"
                    f"▫️ Файл: <code>{out_path.name}</code>\n"
                    f"▫️ Размер: <code>{size_kb} KB</code>{note}"
                ),
                parse_mode="html",
            )
            ...
            await event.edit(
                f"⚠ Не удалось отправить архив, но он сохранён локально:\n"
    ...
    async def auto_backup_loop(self):
        ...
        try:
            await self.client.send_file(
                ...
                file=str(out_path),
                caption=(
                    f"🗃 <b>Авто-бэкап TETKO</b>\n"
                    f"▫️ <code>{out_path.name}</code> ({size_kb} KB)"
                ),
                parse_mode="html",
            )
        except Exception as e:
