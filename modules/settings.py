    ...
    async def cmd_mcubinfo(self, event: events.NewMessage.Event) -> None:
        try:
            ...
            await self.kernel.handle_error(
                e, message="TETKO info command error", event=event
            )
...
<blockquote><tg-emoji emoji-id="5397575638146110953">🌎</tg-emoji> <strong>Telethon-TETKO</strong>: <code>{version_telethon}</code>

<tg-emoji emoji-id="5449918202718985124">🌳</tg-emoji> Branch <strong>{branch}</strong>!</blockquote>"""
        banner_url = "https://raw.githubusercontent.com/hairpin01/TETKO/refs/heads/main/img/info.jpg"
