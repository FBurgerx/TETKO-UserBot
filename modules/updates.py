...
class UpdatesMod(loader.ModuleBase):
    ...
    @loader.command(
        "update",
        doc_en="update TETKO from git",
        doc_ru="oбнoвить TETKO из git",
        doc_uk="оновити TETKO з git",
    )
    async def cmd_update(self, event: Event):
        msg = await event.edit("❄️")
        self.log.info("Updating TETKO")
            ...
            if result.returncode == 0:
                ...
                await msg.edit(
                    ...
                    file=InputMediaWebPage(
                        "https://raw.githubusercontent.com/hairpin01/TETKO/refs/heads/main/img/update.png",
                        optional=True,
                    ),
                    invert_media=True,
                )
