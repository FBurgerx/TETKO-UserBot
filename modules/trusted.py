...
class Trusted(Module):
    ...
    async def cmd_trust(self, event, args):
        if not args:
            await event.edit(
                "❌ Укажите ID или @username.\nИспользование: <code>.trust @user</code>",
                parse_mode="html",
            )
            return
        ...
        if uid is None:
            await event.edit(f"❌ Не удалось найти пользователя <code>{args[0]}</code>", parse_mode="html")
            return

        users = self._load()
        if uid in users:
            await event.edit(f"🤷 Пользователь <code>{uid}</code> уже доверенный.", parse_mode="html")
            return

        users.append(uid)
        self._save(users)
        self._push_to_context()
        await event.edit(f"✅ <code>{uid}</code> теперь доверенный.", parse_mode="html")

    @command(name="untrust", doc="Убрать пользователя из доверенных", only_for="owner")
    async def cmd_untrust(self, event, args):
        if not args:
            await event.edit(
                "❌ Укажите ID или @username.\nИспользование: <code>.untrust @user</code>",
                parse_mode="html",
            )
            return
        ...
        users = self._load()
        if uid not in users:
            await event.edit(f"🤷 Пользователь <code>{uid}</code> и так не доверенный.", parse_mode="html")
            return

        users.remove(uid)
        self._save(users)
        self._push_to_context()
        await event.edit(f"🗑 Пользователь <code>{uid}</code> больше не доверенный.", parse_mode="html")
    ...
    async def cmd_trusted(self, event, args):
        ...
        if not users:
            await event.edit(
                "👥 <b>Доверённые</b>\n\nСписок пуст.\n\n"
                "<i>Добавить:</i> <code>.trust @user</code>",
                parse_mode="html",
            )
            return
        ...
        await event.edit(
            f"👥 <b>Доверённые</b> ({len(users)})\n\n"
            + "\n".join(lines)
            + f"\n\n<i>Владелец:</i> <code>{owner}</code>",
            parse_mode="html",
        )
