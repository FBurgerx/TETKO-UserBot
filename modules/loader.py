...
class Loader(ModuleBase):
    ...
    def _build_module_type_text(
        ...
        *,
        ...
        """Return the compatibility layer used to run the module.

        This text is used by the loader installation result, so it is
        intentionally human-readable and explicit rather than a short label.
        The `.tek <module>` view keeps the compact translator name.
        """
        if hikka_compat or hikka_library:
            return "This module was working in hikka_compat translator."

        obj = getattr(module_obj, "_class_instance", None) or module_obj
        if getattr(obj, "_hikka_compat", False):
            return "This module was working in hikka_compat translator."

        source = ""
        path = getattr(module_obj, "__file__", None) or getattr(obj, "__file__", None)
        if path:
            try:
        ...
        if any(marker in source for marker in mcub_markers):
            return "This module was working in mcub_compat_beta translator."
